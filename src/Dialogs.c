    ...
    case WM_NOTIFY:
      {
        ...
        switch (pnmhdr->code) {
          ...
          case NM_RETURN:
            {
              if (pnmhdr->idFrom == IDC_WEBPAGE_EN) {
                ShellExecute(hwnd,L"open",L"http://www.flos-freeware.ch",NULL,NULL,SW_SHOWNORMAL);
              }
              else if (pnmhdr->idFrom == IDC_WEBPAGE_PL) {
                ShellExecute(hwnd,L"open",L"http://knapsu.eu/notepad2_pl/",NULL,NULL,SW_SHOWNORMAL);
              }
              else if (pnmhdr->idFrom == IDC_EMAIL_EN) {
                ShellExecute(hwnd,L"open",L"mailto:florian.balmer@gmail.com",NULL,NULL,SW_SHOWNORMAL);
              }
              else if (pnmhdr->idFrom == IDC_EMAIL_PL) {
                ShellExecute(hwnd,L"open",L"mailto:knapsu@gmail.com",NULL,NULL,SW_SHOWNORMAL);
              }
            }
            break;
        }
