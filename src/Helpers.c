...
//==== Registry Notepad Replace ===============================================
VOID GetExePath(WCHAR * szExePath)
{
	GetModuleFileName(NULL, szExePath, MAX_PATH);
}

VOID ActivateRegistryNotepadReplace(VOID)
{
  HKEY key;
  LPCWSTR keyPath = L"SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Image File Execution Options\\notepad.exe";
  LSTATUS status;
  WCHAR keyData[MAX_PATH];

  GetExePath(keyData);
  lstrcat(keyData, L" /z");

  status = RegCreateKeyEx(HKEY_LOCAL_MACHINE, keyPath, (DWORD) 0, NULL, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL, &key, NULL);
	if (status == ERROR_SUCCESS) {
		status = RegSetValueEx(key, L"Debugger", 0, REG_SZ, (CONST BYTE *) keyData, (DWORD) lstrlen(keyData) * 2);
	}

  if (status != ERROR_SUCCESS) {
    MessageBox(NULL, L"Operacja zakoñczona niepowodzeniem.", L"", MB_OK);
  }
}

VOID DeactivateRegistryNotepadReplace(VOID)
{
  LPCWSTR keyPath = L"SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Image File Execution Options\\notepad.exe";
  LSTATUS status;

  status = RegDeleteKey(HKEY_LOCAL_MACHINE, keyPath);
...
BOOL IsNotepadReplaced(VOID)
{
  ...
  LSTATUS status;
  WCHAR keyData[MAX_PATH];
  WCHAR keyDataExpected[MAX_PATH];
  DWORD keyDataLength = MAX_PATH;

  GetExePath(keyDataExpected);
  lstrcat(keyDataExpected, L" /z");

  status = RegOpenKeyEx(HKEY_LOCAL_MACHINE, keyPath, 0, KEY_READ, &key);
  if (status == ERROR_SUCCESS) {
    status = RegQueryValueEx(key, L"Debugger", NULL, NULL, (LPBYTE) keyData, (LPDWORD) &keyDataLength);
    if (status == ERROR_SUCCESS) {
      if (lstrcmpi(keyDataExpected, keyData) == 0) {
        bReplaced = TRUE;
      }
    }
  }
...
VOID ToggleNotepadReplace(BOOL bReplace, HWND hWnd)
{
  ...
  if (IsVista()) {
    GetModuleFileName(NULL, szExePath, MAX_PATH);
    if (bReplace) {
      ShellExecute(NULL, L"runas", szExePath, L" /1", 0, SW_SHOWNORMAL);
    } else {
      ShellExecute(NULL, L"runas", szExePath, L" /2", 0, SW_SHOWNORMAL);
    }
  } else {
    if (bReplace) {
      ActivateRegistryNotepadReplace();
    } else {
      DeactivateRegistryNotepadReplace();
    }
  }
